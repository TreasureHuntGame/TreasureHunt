...
instalar_pacote() {
	...
	for gerenciador in "${pkg_managers[@]}"; do 
		...
		# verifica se o gerenciador de pacotes pode ser executado
...
# Se não tiver permissões de superusuário
if [ "$EUID" -ne 0 ]; then
	finalizar_script "Permissões de superusuário necessárias para instalar os pacotes!"
fi
...
while IFS="" read -r pacote; do
	if [ $pacote == "mysql-server" ] && [ ! hash mysql 2>/dev/null]; then # se o mysql não existir na maquina
		...
		echo -e "\nPor favor mude a senha do mysql, para isso execute o seguinte comando (superusuário):"
