...
- [:wheelchair: Acessibilidade](#wheelchair-Acessibilidade-na-Interface-web)
...
- *Nota 9*: Caso o _script_ aponte o erro `caesar: not found` ao gerar problemas com o `caesar`, verifique em que diretório este programa foi instalado (sugestão de comando: `which caesar`) e atualize todas as chamadas a `caesar` nos _scripts_ do jogo para `diretorio/completo/do/caesar`. Isto pode ser feito manualmente ou de forma automática via linha de comando. Por exemplo (executado a partir do diretório [`Jogo`](/Jogo/)): `x=$(which caesar); grep -rl 'caesar' * | xargs sed -i "s#caesar#${x}#g"`. O arquivo [`Fix-caesar-not-found.sh`](/Jogo/Scripts/Fix-caesar-not-found.sh), disponibilizado na pasta [`Scripts`](/Jogo/Scripts/), pode ser executado para resolver este problema. Se `which caesar` retornar vazio, provavelmente a pasta do `caesar` não está no `PATH`. Para resolver isso, adicione ao arquivo `~/.bashrc` ou `~/.bash_profile` o caminho do binário do `caesar` ao `PATH` usando `export PATH="/diretorio/completo/do/caesar/:${PATH}"`. Execute `which caesar` novamente e, se tudo estiver correto, o comando deve retornar o caminho do binário do `caesar`.
